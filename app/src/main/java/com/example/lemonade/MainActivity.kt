...
//            false
    ...
    private fun clickLemonImage() {
        when(lemonadeState){
            ...
            SQUEEZE -> {squeezeCount += 1; lemonSize -= 1; if(lemonSize==0) {lemonadeState=DRINK;lemonSize=-1} else lemonadeState=SQUEEZE;}
            DRINK -> {lemonadeState=RESTART; lemonSize =-1}
    ...
    private fun setViewElements() {
       ...
       when (lemonadeState){
           ...
           else -> {textAction.setText(R.string.lemon_empty_glass); lemonImage?.setImageResource(R.drawable.lemon_restart) }
